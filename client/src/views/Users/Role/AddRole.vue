...
onMounted(() => {
  !allPermissions.value.length > 0 ? store.dispatch('PERMISSION_LIST') : permissionValueSet()
});
...
<template>
  <div>
    ...
    <!-- End Page Header -->
    <a-card title="Create New Role" :bordered="false">
      <a-form ref="formRef" :model="formState" autocomplete="off" @finish="onSubmit" :rules="rules">
        ...
        <a-row class="mt-4">
          <a-col :span="24" style="text-align: right">
            <a-button type="primary" html-type="submit" :loading="submitBtn" :disabled="submitBtn">Submit</a-button>
          </a-col>
        </a-row>
      </a-form>
    </a-card>
